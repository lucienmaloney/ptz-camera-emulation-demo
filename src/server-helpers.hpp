...
translate_seek (ReplayBin * self, GstPad * pad, GstEvent * ievent)
{
  ...
  flags = (GstSeekFlags)((int)flags | GST_SEEK_FLAG_SEGMENT);
...
translate_segment (GstPad * pad, GstEvent * ievent)
{
  ...
  if (!self->outgoing_seek) {
    ...
    gst_segment_do_seek (&segment, 1.0, GST_FORMAT_TIME, (GstSeekFlags)0, GST_SEEK_TYPE_SET,
...
handle_segment_done (ReplayBin * self, GstPad * pad)
{
  ...
  if (self->remainder < INTERVAL) {
    ...
    event =
        ...
        (GstSeekFlags)(self->segment.flags & ~GST_SEEK_FLAG_FLUSH), GST_SEEK_TYPE_SET, ostart,
...
create_replay_bin (GstElement * parent)
{
  ...
  g_object_set (src, "location", filename.c_str(), NULL);
...
{
  GError *error = NULL;
  GOptionContext* optctx = g_option_context_new("");
